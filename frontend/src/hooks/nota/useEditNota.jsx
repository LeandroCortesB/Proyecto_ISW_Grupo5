...
const useEditNota = (setNotas) => {
    ...
    const handleUpdate = async (updatedNotaData) => {
        if (updatedNotaData) {
            ...
            try {
                const dataToUpdate = { ...updatedNotaData }; 
                console.log("Datos de nota actualizados:", dataToUpdate);

                const updatedNota = await updateNota(dataToUpdate, Number(dataNota[0].idNota));
